...
- 不要在回复中粘贴完整 diff、完整文件内容或长日志。
- 默认只输出：
  1) 变更摘要（3-6条）
  2) 受影响文件列表
  3) 我应该运行的命令（如 tests / build）/ 或者下一步的建议。
  4) 本次代码相关的技术点（帮助用户理解学习提升自己本身的能力）
- 如果需要我查看改动，提示我使用 `/diff`（而不是把 diff 贴出来）。
- 若必须展示代码，只展示“最关键的一个 hunk”，最多 40 行，并标注文件路径与行号范围。
